...
G = apply(E, 2, function(x) sum(x > 0)) # where G is the number of unique genes detected for a set of cells
...
HK_set = row.names(E) %in% HK # Mark rows in E as TRUE if the row name is also present in the vector 'HK'
HK_tpm = E[HK_set, ] # Subset our original expression matrix to contain only genes present in the vector 'HK'
...
names(HK_long) = c("Cell", "E") # rename the variables of this data frame
...
Ef_logical = (G >= 1700) | HK_long$E >= 3 # where Ef represents the filtered expression matrix (this returns a logical vector)
Ef = E[ ,Ef_logical]
...
Ep_logical = (Ep >= 4)
Ef = Ef[Ep_logical, ] # omitted genes with very small frequency of detection
...
G_filter = apply(Er, 2, function(x) sum(x > 0))
